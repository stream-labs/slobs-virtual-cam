...
public:
	...
	static OSStatus StreamDeckCueTo(CMIOHardwarePlugInRef self, CMIOStreamID stream, Float64 frameNumber, Boolean playOnCue);
};

PluginInterface::PluginInterface() : ObjectInterface(), m_objectID(0)
{
...
HRESULT PluginInterface::QueryInterface(REFIID uuid, LPVOID *interface)
{
	...
	if (uuidEqual(uuid, kCMIOHardwarePlugInInterfaceID) || uuidEqual(uuid, IUnknownUUID)) {
...
OSStatus PluginInterface::InitializeWithObjectID(CMIOObjectID objectID)
{
	...
	if (device && !device->deviceID.empty())
		this->deviceAdded(this, device->deviceID, device->name, device->width, device->height, device->fps);
...
void PluginInterface::deviceAdded(void *userData, const std::string &deviceId, const std::string &name, uint32_t width, uint32_t height, double fps)
{
	PrintFunction();
...
void PluginInterface::deviceRemoved(void *userData, const std::string &deviceId)
{
	PrintFunction();
...
void PluginInterface::frameReady(void *userData, const std::string &deviceId, const uint8_t *data)
{
	auto self = reinterpret_cast<PluginInterface *>(userData);
...
void PluginInterface::setMirror(void *userData, const std::string &deviceId, bool horizontalMirror, bool verticalMirror)
{
	PrintFunction();
	auto self = reinterpret_cast<PluginInterface *>(userData);
...
bool PluginInterface::createDevice(const std::string &deviceId, const std::string &name, uint32_t width, uint32_t height, double fps)
{
	PrintFunction();
	StreamPtr stream;
	FrameInfo fi;
	...
	device->properties().setProperty(kCMIOObjectPropertyName, name.c_str());
	device->properties().setProperty(kCMIOObjectPropertyManufacturer, vcam_vendor.c_str());
	device->properties().setProperty(kCMIODevicePropertyModelUID, vcam_product.c_str());
	...
	device->properties().setProperty(kCMIODevicePropertySuspendedByUser, UInt32(0));
	device->properties().setProperty(kCMIODevicePropertyHogMode, pid_t(-1), false);
	device->properties().setProperty(kCMIODevicePropertyDeviceMaster, pid_t(-1));
	device->properties().setProperty(kCMIODevicePropertyExcludeNonDALAccess, UInt32(0));
	device->properties().setProperty(kCMIODevicePropertyDeviceIsAlive, UInt32(1));
	device->properties().setProperty(kCMIODevicePropertyDeviceUID, deviceId.c_str());
	device->properties().setProperty(kCMIODevicePropertyTransportType, UInt32(kIOAudioDeviceTransportTypePCI));
	device->properties().setProperty(kCMIODevicePropertyDeviceIsRunningSomewhere, UInt32(0));
	...
	device->setMirror(this->d->ipc_server.isHorizontalMirrored(deviceId), this->d->ipc_server.isVerticalMirrored(deviceId));
	return true;
...
void PluginInterface::destroyDevice(const std::string &deviceId)
{
	...
	for (auto it = this->m_devices.begin(); it != this->m_devices.end(); it++) {
		...
		device->properties().getProperty(kCMIODevicePropertyDeviceUID, &curDeviceId);
...
void PluginInterfacePrivate::updateDevices()
{
	...
	for (auto &device : this->self->m_devices) {
		device->setMirror(this->ipc_server.isHorizontalMirrored(device->deviceId()), this->ipc_server.isVerticalMirrored(device->deviceId()));
	}
}

HRESULT PluginInterfacePrivate::QueryInterface(void *self, REFIID uuid, LPVOID *interface)
{
	PrintFunction();
...
OSStatus PluginInterfacePrivate::InitializeWithObjectID(CMIOHardwarePlugInRef self, CMIOObjectID objectID)
{
	PrintFunction();
...
void PluginInterfacePrivate::ObjectShow(CMIOHardwarePlugInRef self, CMIOObjectID objectID)
{
...
Boolean PluginInterfacePrivate::ObjectHasProperty(CMIOHardwarePlugInRef self, CMIOObjectID objectID, const CMIOObjectPropertyAddress *address)
{
	// PrintFunctionID(objectID);
	PrintFunction();
							  ...
							  Boolean *isSettable)
{
	...
	if (_self->self->objectID() == objectID)
		status = _self->self->isPropertySettable(address, isSettable);
	else if (auto object = _self->self->findObject(objectID))
		status = object->isPropertySettable(address, isSettable);
							   ...
							   UInt32 qualifierDataSize, const void *qualifierData, UInt32 *dataSize)
{
	...
	if (_self->self->objectID() == objectID)
		status = _self->self->getPropertyDataSize(address, qualifierDataSize, qualifierData, dataSize);
	else if (auto object = _self->self->findObject(objectID))
		status = object->getPropertyDataSize(address, qualifierDataSize, qualifierData, dataSize);
						       ...
						       UInt32 qualifierDataSize, const void *qualifierData, UInt32 dataSize, UInt32 *dataUsed, void *data)
{
	...
	if (_self->self->objectID() == objectID)
		status = _self->self->getPropertyData(address, qualifierDataSize, qualifierData, dataSize, dataUsed, data);
	else if (auto object = _self->self->findObject(objectID))
		status = object->getPropertyData(address, qualifierDataSize, qualifierData, dataSize, dataUsed, data);
						       ...
						       UInt32 qualifierDataSize, const void *qualifierData, UInt32 dataSize, const void *data)
{
	...
	if (_self->self->objectID() == objectID)
		status = _self->self->setPropertyData(address, qualifierDataSize, qualifierData, dataSize, data);
	else if (auto object = _self->self->findObject(objectID))
		status = object->setPropertyData(address, qualifierDataSize, qualifierData, dataSize, data);

	return status;
}

OSStatus PluginInterfacePrivate::DeviceSuspend(CMIOHardwarePlugInRef self, CMIODeviceID device)
{
	PrintFunction();
	OSStatus status = kCMIOHardwareUnspecifiedError;
...
OSStatus PluginInterfacePrivate::DeviceResume(CMIOHardwarePlugInRef self, CMIODeviceID device)
{
	PrintFunction();
	OSStatus status = kCMIOHardwareUnspecifiedError;
...
OSStatus PluginInterfacePrivate::DeviceStartStream(CMIOHardwarePlugInRef self, CMIODeviceID device, CMIOStreamID stream)
{
	PrintFunction();
	OSStatus status = kCMIOHardwareUnspecifiedError;
...
OSStatus PluginInterfacePrivate::DeviceStopStream(CMIOHardwarePlugInRef self, CMIODeviceID device, CMIOStreamID stream)
{
	PrintFunction();
	OSStatus status = kCMIOHardwareUnspecifiedError;
...
OSStatus PluginInterfacePrivate::DeviceProcessAVCCommand(CMIOHardwarePlugInRef self, CMIODeviceID device, CMIODeviceAVCCommand *ioAVCCommand)
{
	PrintFunction();
	OSStatus status = kCMIOHardwareUnspecifiedError;
...
OSStatus PluginInterfacePrivate::DeviceProcessRS422Command(CMIOHardwarePlugInRef self, CMIODeviceID device, CMIODeviceRS422Command *ioRS422Command)
{
	PrintFunction();
	OSStatus status = kCMIOHardwareUnspecifiedError;
						       ...
						       void *queueAlteredRefCon, CMSimpleQueueRef *queue)
{
	...
	if (object)
		status = object->copyBufferQueue(queueAlteredProc, queueAlteredRefCon, queue);

	return status;
}

OSStatus PluginInterfacePrivate::StreamDeckPlay(CMIOHardwarePlugInRef self, CMIOStreamID stream)
{
	PrintFunction();
	OSStatus status = kCMIOHardwareUnspecifiedError;
...
OSStatus PluginInterfacePrivate::StreamDeckStop(CMIOHardwarePlugInRef self, CMIOStreamID stream)
{
	PrintFunction();
	OSStatus status = kCMIOHardwareUnspecifiedError;
...
OSStatus PluginInterfacePrivate::StreamDeckJog(CMIOHardwarePlugInRef self, CMIOStreamID stream, SInt32 speed)
{
	PrintFunction();
	OSStatus status = kCMIOHardwareUnspecifiedError;
...
OSStatus PluginInterfacePrivate::StreamDeckCueTo(CMIOHardwarePlugInRef self, CMIOStreamID stream, Float64 frameNumber, Boolean playOnCue)
{
	PrintFunction();
	OSStatus status = kCMIOHardwareUnspecifiedError;
